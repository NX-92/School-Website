...
<body>
    ...
    <div class="container">
        <div class="title">
            <h2>Quizzes</h2>
            <input type="text" placeholder="Search..." id="search" class="search-bar" onkeyup="search()">
        </div>
                ...
                <div class="material-container">
                    ...
                    ?>
                                <div class="materials-context">
                                   <form id="form-<?php echo $learning_Quiz_Data['Quiz_ID']; ?>" action="quiz-page.php" method="post">
                                        <input type="hidden" name="Quiz_ID" value="<?php echo $learning_Quiz_Data['Quiz_ID']; ?>">
                                    </form>
                                    <h3><a href="#" onclick="document.getElementById('form-<?php echo $learning_Quiz_Data['Quiz_ID']; ?>').submit(); return false;">
                                        <span><?php echo $learning_Quiz_Data['Quiz_Name']; ?></span>
                                    </a></h3>
                                    <div class="details">
                                        <p><?php echo htmlspecialchars($learning_Quiz_Data['Description']); ?></p>
                                    </div>
                                    <?php
                                        $quiz_ID = $learning_Quiz_Data['Quiz_ID'];
                                        $attempt_query = "SELECT * FROM attempt WHERE Quiz_ID = '$quiz_ID' AND (Student_ID = '$user_ID' OR Lecturer_ID = '$user_ID') ORDER BY Score DESC LIMIT 1";
                                        $attempt_result = mysqli_query($connection, $attempt_query);

                                        if ($attempt_data = mysqli_fetch_assoc($attempt_result)) {
                                            if ($attempt_data['Score'] >= 50) { ?>
                                                <div class="result-pass">
                                                    <h1><?php echo htmlspecialchars($attempt_data['Grade']); ?></h1>
                                                </div>
                                            <?php } else { ?> 
